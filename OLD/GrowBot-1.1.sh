...
Water()
	{
       NextFlood=$(echo "SELECT DATE_FORMAT(NextFlood, '%Y-%m-%d %H:%i:%s') FROM farmSched order by date desc limit 1" | mysql -upi -pa-51d41e Farm -$
       LastFlood=$(echo "SELECT DATE_FORMAT(LastFlood, '%Y-%m-%d %H:%i:%s') FROM farmSched order by date desc limit 1" | mysql -upi -pa-51d41e Farm -$
       now=$(date +%F-%H:%M:%S)
        if [[ "$now" > "$NextFlood" ]]  || [[ $NextFlood = "00:00:00" ]] || [[ -z $NextFlood ]]
        then
                BedFill=$(date +%T)
                in4=$(date +%T -d "+"$FloodTime"minutes")
                while [[ $(date +%T) < $in4 ]] # ||  [ $(/var/www/hall.py) -eq 0 ]
                do
                        now=$(date +"%D %T")
                        WLeft=$(echo "$(($(date -d "$in4" '+%s') - $(date -d "$now" '+%s')))")
                        if [ $(gpio -g read 15) -eq 0 ]
                        then
                                gpio -g write 15 1
                        fi
                        if [ $(gpio -g read 15) == "0" ]; then PumpStatus="off";else PumpStatus="on";fi
                        Level=$(echo $Level + 5.88 | bc )
                        Max=$Level
                        Sscreen
                done
                LastFlood=$(date +%F-%H:%M:%S)
                NextFlood=$(date +%F-%H:%M:%S -d "+"$DryTime"minutes")
                line=$(echo "'"$(date +%F-%H:%M:%S)"','"$LastFlood"','"$NextFlood"','"$DryTime"'")
                lineFixed=$(echo "'"$(date +%F-%H:%M:%S)"','"$LastFlood"','"$NextFlood"','"$DryTime"'" | sed -r 's/\//-/g')
                echo "INSERT INTO farmSched (date,LastFlood,NextFlood,period) VALUES ("$line ");" | mysql -upi -pa-51d41e Farm -N
                gpio -g write 15 0
        elif [[ $Level > "0" ]]
        then

                while [[ $Level != "0" ]] # || [[ $(/var/www/hall.py) -eq 1 ]]
                do
