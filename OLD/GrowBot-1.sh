...
Water()
	{
        ...
        now=$(date +"%D %T")
	NextFlood=$(echo "SELECT DATE_FORMAT(NextFlood, '%m/%d/%Y %H:%i') FROM farmSched order by date desc limit 1" | mysql -upi -pa-51d41e Farm -N)
	echo "Now: "$now " NextFlood: "$NextFlood && sleep 5
        if [[ $now > $NextFlood ]] || [[ $ForceFlood -eq "1" ]] || [[ $NextFlood = "00:00:00" ]]
        then
                FillStart=$(date +"%D %T")
                in3=$(date +%T -d "+6minutes")
		...
		Vol=$(echo "SELECT Vol FROM farmSched ORDER BY date DESC LIMIT 1;" | mysql -upi -pa-51d41e Farm -N )
        	line=$(echo "'"$(date +%F-%H:%M:%S)"','"$DrainStop"','"$NextFlood"','"$period"','"$LastpHCal"','"$NextpHCal"','"$Vol"'")
		echo $line && sleep 10
